...
export default function Navigation() {
  ...
  return (
    <header className="flex w-full justify-between items-center py-5 border-b border-[var(--border)] md:px-12 px-5 fixed top-0 z-[3000] backdrop-blur-xl shadow-sm bg-white/90">
      ...
      <nav className="hidden lg:flex items-center justify-center gap-1">
        <div className="relative">
          ...
          {showSolutions && (
            <div
              className="absolute top-11 -left-4 w-72 border border-[var(--border)] rounded-xl px-2 py-3 shadow-lg bg-white flex flex-col gap-1 z-[3001]"
        ...
        <Link href="/tools" className="px-4 py-2 rounded-md text-gray-700 font-semibold text-sm hover:bg-gray-100 transition-colors">Free Tools</Link>
        <Link href="/blog" className="px-4 py-2 rounded-md text-gray-700 font-semibold text-sm hover:bg-gray-100 transition-colors">Blog</Link>
        <Link href="/pricing" className="px-4 py-2 rounded-md text-gray-700 font-semibold text-sm hover:bg-gray-100 transition-colors">Pricing</Link>
      </nav>

      <div className="hidden lg:flex items-center gap-3">
        <a href="https://discord.gg/classroomio" target="_blank" rel="noopener noreferrer" aria-label="Discord" className="flex items-center hover:opacity-75 transition-opacity">
        ...
        <a href="https://github.com/classroomio/classroomio" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md hover:bg-gray-100 transition-colors" aria-label="GitHub">
        ...
        <a href="https://app.classroomio.com" className="font-medium text-sm text-gray-800 hover:opacity-75 transition-opacity after:content-['_→']">
          Dashboard
        </a>
      </div>

      <button type="button" aria-label="Open menu" className="block lg:hidden" onClick={() => setShowMobileMenu((v) => !v)}>
        {showMobileMenu ? <X size={24} /> : <Menu size={24} />}
      </button>

      {showMobileMenu && (
        <div className="absolute right-0 top-0 w-[70%] md:w-[45%] h-screen border-l border-[var(--border)] bg-white z-[3002] px-4 pt-4 pb-6 flex flex-col shadow-xl">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowMobileMenu(false)} aria-label="Close menu"><X size={22} /></button>
          </div>
          <nav className="flex flex-col gap-1 flex-1">
            ...
            {showMobileSolutions && (
              <div className="pl-4 flex flex-col gap-1 mb-1">
                {solutions.map((s) => (
                  <Link key={s.key} href={`/${s.key}`} className="text-sm text-gray-600 hover:bg-gray-50 py-2.5 px-3 rounded-lg transition-colors" onClick={() => setShowMobileMenu(false)}>
                    {s.title}
                  </Link>
                ))}
              </div>
            ...
            {[
              ...
              <Link key={item.href} href={item.href} className="py-3 px-3 rounded-lg hover:bg-gray-50 font-semibold text-sm text-gray-800 transition-colors" onClick={() => setShowMobileMenu(false)}>
                {item.label}
              </Link>
            ))}
          </nav>
          <div className="border-t border-[var(--border)] pt-4 flex flex-col gap-2">
            <a href="https://app.classroomio.com" className="py-3 px-3 rounded-lg hover:bg-gray-50 font-semibold text-sm text-gray-800 transition-colors after:content-['_→']">
              Dashboard
            </a>
          </div>
        </div>
