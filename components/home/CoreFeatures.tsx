const testimonial = {
  avatar: "https://senjaio.b-cdn.net/public/avatar/406f29d4-5811-4f2e-90c1-e586fce3b3a7_me.JPEG?width=78&height=78&format=webp",
  quote: "ClassroomIO really helped me bootstrap my program. The dashboard is extremely intuitive. I could navigate most of it without any help / support. Coming from framer, I can honestly say that I am extremely grateful that I found ClassroomIO.",
  name: "Pranav Singhal",
...
function FeatureSection({ id, tagline, title, description, video, rightToLeft }: (typeof sections)[0]) {
  return (
    <section id={id} className="w-full py-24 border-b border-gray-200">
      <div className={`w-4/5 mx-auto flex flex-col ${rightToLeft ? "lg:flex-row-reverse" : "lg:flex-row"} items-center justify-between gap-10 lg:gap-20`}>
        <div className="w-full lg:w-1/2 flex flex-col">
          <p className="text-sm font-medium text-gray-500 mb-2 uppercase tracking-wider">{tagline}</p>
        ...
        <div className="w-full lg:w-1/2">
          <video width="100%" height="100%" className="w-full h-auto rounded-xl shadow-xl" autoPlay loop muted playsInline preload="auto">
    ...
    <>
      <section className="w-full py-24 border-b border-gray-200">
        <div className="max-w-2xl mx-auto flex flex-col items-center text-center px-6">
          ...
          <img src={testimonial.avatar} alt={`${testimonial.name} avatar`} className="w-14 h-14 rounded-full mb-4 object-cover" loading="lazy" referrerPolicy="no-referrer" />
          ...
          <a href={testimonial.roleLink} target="_blank" rel="noopener noreferrer" className="text-xs text-gray-400 hover:text-gray-600">{testimonial.role}</a>
        </div>
      </section>
      {sections.map((s) => (
